...
<c:if test="${sessionScope.member.userId==dto.hostId || sessionScope.member.userId=='admin'}">
	function deleteApply() {
	    if(confirm("게시글을 삭제 하시 겠습니까 ? ")) {
		    let query = "camMateAppNum=${dto.camMateAppNum}&$page=${page}";
		    let url = "${pageContext.request.contextPath}/mate/applydelete.do?" + query;
	    	location.href = url;
	    }
	}
</c:if>
...
<c:if test="${sessionScope.member.userId==dto.hostId || sessionScope.member.userId=='admin'}">
	function listApply() {
		let query = "camMateNum=${dto.camMateNum}&$page=${page}";
		    let url = "${pageContext.request.contextPath}/mate/applylist.do?" + query;
	    	location.href = url;
	}
</c:if>
