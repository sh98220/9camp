...
<c:if test="${sessionScope.member.userId!=dto.hostId && sessionScope.member.userId!='admin'}">
	function applyMate() {
	    if(confirm("메이트가 되시겠습니까 ? ")) {
		    let query = "camMateNum=${dto.camMateNum}&page=${page}";
		    let url = "${pageContext.request.contextPath}/mate/apply.do?" + query;
	    	location.href = url;
	    }
	}
</c:if>
...
function deleteMate() {
    if(confirm("게시글을 삭제 하시 겠습니까 ? ")) {
	    let query = "camMateNum=${dto.camMateNum}&page=${page}";
	    let url = "${pageContext.request.contextPath}/mate/delete.do?" + query;
    	location.href = url;
    }
}
</c:if>
	    ...
	    <div class="body-main mx-auto">
			<table class="table table-border table-article">
				...
				<tbody>
					<tr>
						...
						<td align="right">
							${dto.camMateRegDate} | 조회 ${dto.camMateHitCount}
						</td>
					</tr>
					...
					<tr>
						<td width="50%">
							참가회비 : ${dto.camMateDues}
						</td>
					</tr>
					
					<tr>
						<td width="50%">
							기간 : <a href="#"></a> ${dto.camMateStartDate}&nbsp;~&nbsp;${dto.camMateEndDate}
						</td>
					</tr>
					...
					<tr>
						<td colspan="2">
						        ...
						        <c:choose>
						            ...
						            <c:otherwise>
						                <a class="nav-link" href="${pageContext.request.contextPath}/mate/article.do?${query}&camMateNum=${preReadDto.camMateNum}">${preReadDto.camMateSubject}</a>
						            </c:otherwise>
						        </c:choose>
						    </td>
						</tr>
					<tr>
						<td colspan="2">
							...
							<c:choose>
					             ...
					             <c:otherwise>
					                 <a class="nav-link"  href="${pageContext.request.contextPath}/mate/article.do?${query}&camMateNum=${nextReadDto.camMateNum}">${nextReadDto.camMateSubject}</a>
					             </c:otherwise>
					         </c:choose>
						</td>
					</tr>
			...
			<table class="table">
				<tr>
					<td width="50%">
					    <c:choose>
				    		<c:when test="${sessionScope.member.userId!=dto.hostId && sessionScope.member.userId!='admin'}">
				    			<button type="button" class="btn" onclick="applyMate();">지원하기</button>
				    		</c:when>
				    		<c:otherwise>
				    			<button type="button" class="btn" disabled="disabled">지원하기</button>
				    		</c:otherwise>
				    	</c:choose>
					    <c:choose>
				    		<c:when test="${sessionScope.member.userId==dto.hostId || sessionScope.member.userId=='admin'}">
				    			<button type="button" class="btn" onclick="location.href='${pageContext.request.contextPath}/mate/applyList.do?camMateNum=${dto.camMateNum}&page=${page}'">지원자목록</button>
				    		</c:when>
				    		<c:otherwise>
				    			<button type="button" class="btn" disabled="disabled">지원자목록</button>
				    		</c:otherwise>
				    	</c:choose>
				    	
						<c:choose>
							<c:when test="${sessionScope.member.userId==dto.hostId}">
								<button type="button" class="btn" onclick="location.href='${pageContext.request.contextPath}/mate/update.do?camMateNum=${dto.camMateNum}&page=${page}';">수정</button>
							</c:when>
							<c:otherwise>
						...
						<c:choose>
				    		<c:when test="${sessionScope.member.userId==dto.hostId || sessionScope.member.userId=='admin'}">
				    			<button type="button" class="btn" onclick="deleteMate();">삭제</button>
				    		</c:when>
				    		<c:otherwise>
					...
					<td align="right">
						<button type="button" class="btn" onclick="location.href='${pageContext.request.contextPath}/mate/list.do?${query}';">리스트</button>
					</td>
				</tr>
			</table>
	    </div>
