...
function sendOk() {
    ...
    str = f.camAppSubject.value.trim();
    if(!str) {
        alert("제목을 입력하세요. ");
        f.camAppSubject.focus();
        return;
    }

    str = f.camAppContent.value.trim();
    if(!str) {
        alert("내용을 입력하세요. ");
        f.camAppContent.focus();
        return;
    }

    str = f.camAppAge.value.trim();
    if(!str) {
        alert("나이를 입력하세요. ");
        f.camAppAge.focus();
        return;
    ...
    f.action = "${pageContext.request.contextPath}/mate/apply_ok.do";
    f.submit();
}
</script>
</head>
...
<main>
	<div class="container body-container">
	    <div class="body-title">
			<h2><i class="fas fa-graduation-cap"></i> 캠핑메이트 지원 </h2>
	    </div>
			...
			<form name="boardForm" method="post" enctype="multipart/form-data">
				<table class="table table-border table-form">
					<tr> 
						<td>제&nbsp;&nbsp;&nbsp;&nbsp;목</td>
						<td> 
							<input type="text" name="camAppSubject" maxlength="100" class="form-control" value="${dto.camAppSubject}">
						</td>
					</tr>
					<tr>
						<td>지원자</td>
					...
					<tr> 
						...
						<td> 
							<textarea name="camAppContent" class="form-control">${dto.camAppContent}</textarea>
						</td>
					</tr>
					...
					<tr>
					    <td>성&nbsp;&nbsp;&nbsp;&nbsp;별</td>
					    <td>
					        <input type="radio" name="camMateAppGender"  value="F" checked="checked">F
					        <input type="radio" name="camMateAppGender" value="M">M
					    </td>					   
					</tr>
					<tr>
						<td>나&nbsp;&nbsp;&nbsp;&nbsp;이</td>
						<td>
							<input type="text" name="camAppAge" placeholder="숫자만 입력해주세요."> 
						</td>
					</tr>
				</table>
				...
				<table class="table">
					<tr> 
						<td align="center">
							<button type="button" class="btn" onclick="sendOk();">${mode=="update"?"수정완료":"지원완료"}</button>
							<button type="reset" class="btn">다시입력</button>
							<button type="button" class="btn" onclick="location.href='${pageContext.request.contextPath}/mate/list.do';">${mode=="update"?"수정취소":"지원취소"}</button>
							<input type="hidden" name="camMateNum" value="${dto.camMateNum}">
							<c:if test="${mode=='update' }">
								<input type="hidden" name="camMateAppNum" value="${dto.camMateAppNum}">
